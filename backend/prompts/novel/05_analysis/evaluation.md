---
title: 章节版本评估
description: 章节版本评估和选择的系统提示词，从剧情连贯性、文学性、人物一致性、世界观契合度、伏笔处理和叙事节奏等维度评估多个版本并选出最佳版本
tags: evaluation, quality, selection
---
...
### relevant_context (语义相关的历史内容) - 可选

这是通过RAG（检索增强生成）技术找到的与待评估章节语义最相关的历史内容。包含两部分：

- relevant_chunks：与当前章节内容语义相似的历史文本片段，每个片段包含来源章节号、章节标题、具体内容和相关度评分
- relevant_summaries：与当前章节相关的历史章节摘要

如何使用：

1. **伏笔检测**：检查待评估版本是否正确回收了这些相关片段中提到的伏笔或线索
2. **人物一致性**：对比相关片段中角色的表现，判断待评估版本中同一角色的言行是否一致
3. **细节连贯**：检查场景描写、物品描述、时间线等细节是否与历史内容矛盾
4. **情感延续**：判断角色间的情感发展是否与历史内容中的铺垫相呼应

注意：relevance_score 表示相关度（0-1），分数越高表示与待评估章节越相关，应优先关注高分内容。
