...
import { CheckCircle2, Circle, Image as ImageIcon, Plus, RefreshCw } from 'lucide-react';
      ...
      <div className="flex h-full min-h-0 flex-col gap-4">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto pr-1 custom-scrollbar">
          <ConfigsCardsList
            {...commonListProps}
            renderLeft={(cfg) => {
              const providerLabel = PROVIDER_LABELS[cfg.provider_type] || cfg.provider_type;
              const baseUrl = cfg.api_base_url || '—';
              const model = cfg.model_name || '—';
              const style = cfg.default_style || '—';
              const ratio = cfg.default_ratio || '—';
              ...
              return (
                <>
                  ...
                  <div className="mt-2 grid min-w-0 grid-cols-1 gap-2 text-xs text-book-text-muted sm:grid-cols-2">
                    ...
                    <div className="min-w-0 truncate" title={`默认风格：${style}`}>默认风格：{style}</div>
                    <div className="min-w-0 truncate" title={`默认比例：${ratio}`}>默认比例：{ratio}</div>
                  </div>
                </>
              );
            }}
