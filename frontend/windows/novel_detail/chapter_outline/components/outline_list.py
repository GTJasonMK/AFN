...
from utils.component_pool import ComponentPool, reset_outline_row
...
class OutlineListView(QWidget):
    ...
    def __init__(
        ...
        # 组件池：复用OutlineRow组件
        self._row_pool = ComponentPool(
            OutlineRow,
            max_size=100,
            factory_kwargs={'data': {}, 'row_type': self.item_type},
            reset_callback=reset_outline_row
        )
    ...
    def update_data(self, items: list):
        ...
        for row in self.row_widgets:
            try:
                row.detailClicked.disconnect(self.itemClicked.emit)
            except (TypeError, RuntimeError):
                pass
            if self.item_type == "chapter":
                try:
                    row.editRequested.disconnect(self.editRequested.emit)
                except (TypeError, RuntimeError):
                    pass
            self.content_layout.removeWidget(row)
            self._row_pool.release(row)

        # 清除其他widget（如empty_label、stretch）
        while self.content_layout.count():
            item = self.content_layout.takeAt(0)
            if item.widget() and item.widget() not in self.row_widgets:
