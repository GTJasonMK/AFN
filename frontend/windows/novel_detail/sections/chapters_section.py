...
class ChaptersSection(ThemeAwareWidget):
    ...
    def __init__(self, chapters=None, parent=None):
        ...
        # 保存UI组件引用
        self.chapter_list_view = None  # QListView（虚拟化列表）
        self.chapter_list_model = None  # ChapterListModel（数据模型）
        self.chapter_list_delegate = None  # ChapterItemDelegate（绘制代理）
    ...
    def _createChapterList(self, layout):
        ...
        # 章节列表 - 使用QListView + Model + Delegate实现虚拟化
        self.chapter_list_view = QListView()
        self.chapter_list_view.setObjectName("chapter_list")
        self.chapter_list_view.setSelectionMode(QAbstractItemView.SelectionMode.SingleSelection)
        self.chapter_list_view.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        self.chapter_list_view.setVerticalScrollMode(QAbstractItemView.ScrollMode.ScrollPerPixel)

        # 创建数据模型
        self.chapter_list_model = ChapterListModel(self.completed_chapters, self)
        self.chapter_list_view.setModel(self.chapter_list_model)

        # 创建绘制代理
        self.chapter_list_delegate = ChapterItemDelegate(self)
        self.chapter_list_view.setItemDelegate(self.chapter_list_delegate)

        # 连接选择信号
        self.chapter_list_view.selectionModel().currentRowChanged.connect(
            lambda current, _: self._onChapterSelected(current.row())
        )

        list_layout.addWidget(self.chapter_list_view)
    ...
    def _apply_theme(self):
        ...
        if self.list_container:
            ...
            # 刷新delegate的主题缓存
            if self.chapter_list_delegate:
                self.chapter_list_delegate.refreshTheme()
                # 强制重绘列表
                if self.chapter_list_view:
                    self.chapter_list_view.viewport().update()
        ...
        if self.detail_stack and self.detail_stack.count() > 1:
            # 有显示中的章节详情（不是空状态）
            current_index = -1
            if self.chapter_list_view and self.chapter_list_view.selectionModel():
                current_model_index = self.chapter_list_view.selectionModel().currentIndex()
                current_index = current_model_index.row() if current_model_index.isValid() else -1
    ...
    def updateData(self, new_chapters):
        """更新章节数据并刷新显示 - 优化版：增量更新"""
        self.all_chapters = new_chapters
        old_count = len(self.completed_chapters)
        self._filter_completed_chapters()
        ...
        # 如果布局已存在
        if self.layout():
            # 情况1：从无章节变为有章节，或从有章节变为无章节 - 需要重建UI
            if (old_count == 0 and new_count > 0) or (old_count > 0 and new_count == 0):
                ...
                self.chapter_list_model = None
                self.chapter_list_delegate = None
                self.count_label = None
                self.list_container = None
                self.detail_container = None
                self.detail_stack = None
            ...
            # 情况2：章节数量变化但都有章节 - 增量更新model
            elif self.chapter_list_model:
                # 直接更新model数据，避免重建UI
                self.chapter_list_model.setChapters(self.completed_chapters)

                # 更新章节计数标签
                if self.count_label:
                    self.count_label.setText(f"{new_count} 章")

            # 如果有章节，默认选中第一个
            if self.completed_chapters and self.chapter_list_view:
                index = self.chapter_list_model.index(0, 0)
                self.chapter_list_view.setCurrentIndex(index)
