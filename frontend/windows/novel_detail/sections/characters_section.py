...
from utils.component_pool import ComponentPool, reset_character_row

from ..components import CharacterRow, CharacterPortraitsWidget
...
class CharactersSection(ThemeAwareWidget):
    ...
    def __init__(self, data=None, editable=True, project_id: str = "", parent=None):
        ...
        # 组件池：复用角色行组件
        self._character_row_pool = ComponentPool(
            CharacterRow,
            max_size=50,
            factory_kwargs={'data': {}},
            reset_callback=reset_character_row
        )
    ...
    def _create_character_rows(self):
        ...
        if len(self.data) <= 10:
            for character in self.data:
                row = self._character_row_pool.acquire()
                row.update_data(character)
        ...
        # 对于大量角色，使用分批创建
        self._pending_characters = list(self.data)
        self._character_batch_index = 0
        self._character_batch_size = 5  # 每批创建5个
        self._create_next_character_batch()

    def _create_next_character_batch(self):
        """创建下一批角色行"""
        from PyQt6.QtCore import QTimer
        ...
        start_idx = self._character_batch_index * self._character_batch_size
        end_idx = min(start_idx + self._character_batch_size, len(self._pending_characters))

        if start_idx >= len(self._pending_characters):
            # 所有角色创建完成
        ...
        for i in range(start_idx, end_idx):
            character = self._pending_characters[i]
            row = self._character_row_pool.acquire()
            row.update_data(character)
            self.character_rows.append(row)
            # 插入到stretch之前
        ...
        # 继续创建下一批
        self._character_batch_index += 1
        if self._character_batch_index * self._character_batch_size < len(self._pending_characters):
            QTimer.singleShot(8, self._create_next_character_batch)
        else:
            self._pending_characters = None
    ...
    def updateData(self, new_data):
        ...
        for row in self.character_rows:
            ...
            self._character_row_pool.release(row)

        # 清空其他widget
        while self.content_layout.count():
            item = self.content_layout.takeAt(0)
            if item.widget() and item.widget() not in self.character_rows:
