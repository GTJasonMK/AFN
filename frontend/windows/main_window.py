...
class MainWindow(QMainWindow):
    ...
    def navigateTo(self, page_type: str, params: dict = None):
        ...
        # 检查页面是否已缓存
        cache_key = page_type
        if page_type in ['DETAIL', 'WRITING_DESK', 'CODING_DETAIL']:
        ...
        needs_loading = not is_cached and page_type in ['DETAIL', 'WRITING_DESK', 'CODING_DETAIL']
    ...
    def getOrCreatePage(self, page_type: str, params: dict):
        ...
        Returns:
            页面widget实例
        """
        # 对于需要多实例的页面（如DETAIL, WRITING_DESK, CODING_DETAIL），使用参数作为缓存键
        cache_key = page_type
        if page_type in ['DETAIL', 'WRITING_DESK', 'CODING_DETAIL']:
            project_id = params.get('project_id')
            if not project_id:
